...
HLTDisplacedDmesonVtxFilter::HLTDisplacedDmesonVtxFilter(const edm::ParameterSet& iConfig) :
	...
	CosinePointingAngle3D_(iConfig.getParameter<bool>("CosinePointingAngle3D")),
...
void HLTDisplacedDmesonVtxFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
	...
	desc.add<bool>("CosinePointingAngle3D",true);
...
bool HLTDisplacedDmesonVtxFilter::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
	...
	//if (saveTags()) filterproduct.addCollectionTag("hltIter2MergedForBTag");
	...
	for(reco::VertexCollection::iterator it = displacedVertexColl.begin(); it!= displacedVertexColl.end(); it++)
	{
		...
		//point angle cut
		double agl;
		if( CosinePointingAngle3D_)
		{
			TVector3 psum(vertextkRef1->px() + vertextkRef2->px(), vertextkRef1->py() + vertextkRef2->py(), vertextkRef1->pz() + vertextkRef2->pz());
			TVector3 pvtosv(displacedVertex.x() - vtx.x(), displacedVertex.y() - vtx.y(), displacedVertex.z() - vtx.z());
		    agl = TMath::Cos(psum.Angle(pvtosv));
		}
		else
		{
			TVector3 psum(vertextkRef1->px() + vertextkRef2->px(), vertextkRef1->py() + vertextkRef2->py(), 0.);
