...
#include "HLTDisplacedtracktrackFilter.h"
#include "TMath.h"

//
// constructors and destructor
//
HLTDisplacedtracktrackFilter::HLTDisplacedtracktrackFilter(const edm::ParameterSet& iConfig) :
  HLTFilter(iConfig),
...
HLTDisplacedtracktrackFilter::~HLTDisplacedtracktrackFilter()
{

}

void HLTDisplacedtracktrackFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("DisplacedVertexTag",edm::InputTag("hltDisplacedtracktrackVtx"));
  ...
  descriptions.add("hltDisplacedtracktrackFilter", desc);
 }

// ------------ method called once each job just before starting event loop  ------------
void HLTDisplacedtracktrackFilter::beginJob()
{

}

// ------------ method called once each job just after ending the event loop  ------------
void HLTDisplacedtracktrackFilter::endJob()
{
 	
}

// ------------ method called on each new Event  ------------
bool HLTDisplacedtracktrackFilter::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
  ...
  for(reco::VertexCollection::iterator it = displacedVertexColl.begin(); it!= displacedVertexColl.end(); it++){
          ...
          if(displacedVertex.tracksSize() != 2)  throw cms::Exception("BadLogic") << "HLTDisplacedtracktrackFilter: ERROR: the Jpsi vertex must have exactly two muons by definition. It now has n muons = "
	  ...
	  if(iFoundRefs != 2) throw cms::Exception("BadLogic") << "HLTDisplacedtracktrackFilter: ERROR: the Jpsi vertex must have exactly two muons by definition."  << std::endl;
	  ...
	  GlobalPoint displacementFromBeamspot( -1*((vertexBeamSpot.x0() -  secondaryVertex.x()) +  (secondaryVertex.z() - vertexBeamSpot.z0()) * vertexBeamSpot.dxdz()),
          ...
          //calculate the angle between the decay length and the tracktrack momentum
  ...
  LogDebug("HLTDisplacedtracktrackFilter") << " >>>>> Result of HLTDisplacedtracktrackFilter is "<< triggered <<std::endl;
