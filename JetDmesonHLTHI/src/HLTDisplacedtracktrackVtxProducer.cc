...
#include "HLTDisplacedtracktrackVtxProducer.h"
...
//
HLTDisplacedtracktrackVtxProducer::HLTDisplacedtracktrackVtxProducer(const edm::ParameterSet& iConfig):	
...
HLTDisplacedtracktrackVtxProducer::~HLTDisplacedtracktrackVtxProducer()
{

}

void
HLTDisplacedtracktrackVtxProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add("hltDisplacedtracktrackVtxProducer", desc);
}

// ------------ method called once each job just before starting event loop  ------------
void HLTDisplacedtracktrackVtxProducer::beginJob()
{

}

// ------------ method called once each job just after ending the event loop  ------------
void HLTDisplacedtracktrackVtxProducer::endJob() 
{
 	
}

// ------------ method called on each new Event  ------------
void HLTDisplacedtracktrackVtxProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	for (cand1=mucands->begin(); cand1!=mucands->end(); cand1++) {
	       ...
	       LogDebug("HLTDisplacedtracktrackFilter") << " 1st muon in loop: q*pt= " << cand1->charge()*cand1->pt() << ", eta= " << cand1->eta() << ", hits= " << tk1->numberOfValidHits();
	       ...
	       for (; cand2!=mucands->end(); cand2++) {
			 ...
			 // eta cut
			 LogDebug("HLTDisplacedtracktrackVtxProducer") << " 2nd muon in loop: q*pt= " << cand2->charge()*cand2->pt() << ", eta= " << cand2->eta() << ", hits= " << tk2->numberOfValidHits() << ", d0= " << tk2->d0();
			 //first check if this muon passed the previous filter
			 ...
			 LogDebug("HLTDisplacedtracktrackFilter") << " ... 1-2 invmass= " << invmass;
...
bool HLTDisplacedtracktrackVtxProducer::checkPreviousCand(const TrackRef& trackref, vector<RecoChargedCandidateRef> & refVect){
  bool ok=false;
